...
func main() {
	...
	file, err := os.Create("./8_thread_fps_log.csv")
	if err != nil {
	...
	// These are the partitions that the threads will work on, they are split into two so that the threads can work on different parts of the grid,this time I have split the grid into 8 partitions so I can run 8 threads
	partitions := []Partition{
		{startX: 0, endX: xdim / 8, BoundaryMutex: boundaryMutex},
		{startX: xdim / 8, endX: xdim / 4, BoundaryMutex: boundaryMutex},
		{startX: xdim / 4, endX: 3 * xdim / 8, BoundaryMutex: boundaryMutex},
		{startX: 3 * xdim / 8, endX: xdim / 2, BoundaryMutex: boundaryMutex},
		{startX: xdim / 2, endX: 5 * xdim / 8, BoundaryMutex: boundaryMutex},
		{startX: 5 * xdim / 8, endX: 3 * xdim / 4, BoundaryMutex: boundaryMutex},
		{startX: 3 * xdim / 4, endX: 7 * xdim / 8, BoundaryMutex: boundaryMutex},
		{startX: 7 * xdim / 8, endX: xdim, BoundaryMutex: boundaryMutex},
	}
