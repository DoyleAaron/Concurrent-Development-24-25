...
func main() {
	...
	file, err := os.Create("./4_thread_fps_log.csv")
	if err != nil {
	...
	// These are the partitions that the threads will work on, they are split into two so that the threads can work on different parts of the grid,this time I have split the grid into 4 partitions
	partitions := []Partition{
		{startX: 0, endX: xdim / 4, BoundaryMutex: boundaryMutex},
		{startX: xdim / 4, endX: xdim / 2, BoundaryMutex: boundaryMutex},
		{startX: xdim / 2, endX: 3 * xdim / 4, BoundaryMutex: boundaryMutex},
		{startX: 3 * xdim / 4, endX: xdim, BoundaryMutex: boundaryMutex},
	}
